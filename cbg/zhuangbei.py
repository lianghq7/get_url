...
def main():
    ...
    kindid = input('已选择%s, 请输入类型编号:' %(server_name))
    beg_page = 2
    max_page = 2
    ...
    level_to_xiezi_minjie = {'60': 27, '70': 29, '80': 31, '90': 32, '100': 35, '110': 38, '120': 41, '130': 44, '140': 47, '150': 50, '160': 52}
    




    url1 = 'http://recommd.xyq.cbg.163.com/cgi-bin/recommend.py?&act=recommd_by_role&server_id=' + server_id + '&areaid=' + areaid + '&server_name=' + server_name + '&page=1kindid=' + kindid + '&special_mode=and&hide_lingshi=1&count=15'
    ...
    fp=open(server_name + "-" + kindid_to_name[kindid] + "-" + str(total_pages) + "页.txt","w")
    csvout = open(server_name + "-" + kindid_to_name[kindid] + "-" + str(total_pages) + "页.csv", "w", newline='')
    ...
    '永不磨损', '简易', '暴怒', '神农', '神佑', '精致', '绝杀', '专注', '必中', '160级装备特性', '开运孔数', '星位', '套装效果', '人造']
    ...
    for page in range(1, total_pages + 1):
        ...
        each_15 = final.split('equip_level_desc')
        del each_15[0]
        for each in each_15:
            hole_num = '0'
            failtime = '0'
            ...
            # print (each)
            equip_level_desc = re.findall(r'\d+级", "price_int"', each)[0].replace('", "price_int"', '').replace('级', '')
            print(equip_level_desc)
            price_int = re.findall(r'"price_int": \d+, ', each)[0].replace('"price_int": ', '').replace(',', '').replace(' ', '')
            print(price_int)
            server_id = re.findall(r'"server_id": \d+, ', each)[0].replace('"server_id": ', '').replace(',', '').replace(' ', '')
            print(server_id)
            if (len(re.findall(r'"hole_num": \d+, ', each)) > 0):
                hole_num = re.findall(r'"hole_num": \d+, ', each)[0].replace('"hole_num": ', '').replace(',', '').replace(' ', '')
            print(hole_num)
            # temp_text = each
            # juese_begin = each.find("【装备角色】")
            # juese_end = each.find("expire_time")
            # temp_text = temp_text[juese_begin + 6:juese_end - 1]
            # juese = temp_text.replace('",', '').replace(' ', '')
            # # juese = re.findall(r'【装备角色】.+"expire_time"', each)[0].replace('【装备角色】', '').replace('",', '').replace('"expire_time"', '').replace(' ', '')
            # print(juese)
            kindid = re.findall(r'"kindid": \d+, ', each)[0].replace('"kindid":', '').replace(',', '').replace(' ', '')
            print(kindid)
            if (len(re.findall(r'制造者：', each)) > 0):
                renzao = '1'
            else:
                renzao = '0'
            print(renzao)
            if (len(re.findall(r'修理失败 \d次', each)) > 0):
                failtime = re.findall(r'修理失败 \d次', each)[0].replace('修理失败 ', '').replace('次', '')
            print(failtime)
            equip_name = re.findall(r'"equip_name": ".+?",', each)[0].replace('"equip_name": "', '').replace('",', '')
            print(equip_name)
            if (len(re.findall(r'锻炼等级 \d+', each)) > 0):
                baoshi_level = re.findall(r'锻炼等级 \d+', each)[0].replace('锻炼等级 ', '')
                baoshi = re.findall(r'镶嵌宝石 .+?#', each)[0].replace('镶嵌宝石 ', '').replace('#', '')
            ...
            if (len(re.findall(r'星位：', each)) > 0):
                ...
                if (level_int > 120):
                    ...
                    kaikong = '2'
            elif (len(re.findall(r'"hole_num": \d,', each)) > 0):
                kaikong = re.findall(r'"hole_num": \d,', each)[0].replace('"hole_num": ', '').replace(',', '')

            print(xingwei)
            #shuxingdian
            temp_text = each
            ronglian_begin = each.find("熔炼效果：")
            fushi_begin = each.find("符石:")
            ...
            if (len(re.findall(r'(#r| )伤害 \+\d+', temp_text)) > 0):
                shanghai = re.findall(r'(#r| )伤害 \+\d+', temp_text)[0].replace('#r伤害 +', '').replace(' 伤害 +', '')
                print ('伤害:', shanghai)
    ...
    # 修罗咒 天衣无缝 冥王暴杀 乾坤斩 帝释无双 伽罗无双 亡灵之刃 死亡之音 身似菩提 心如明镜 移形换影 凝心决 毁灭之光 金刚不坏
            ...
            writer.writerow({'equip_level':equip_level_desc,'price_int':price_int,'server_id': server_id, 
                ...
                '黑宝石': heibaoshi, '神秘石': shenmishi, '翡翠石': feicuishi,'红宝石': hongbaoshi, '红玛瑙加成': hongmanaijiacheng,
                ...
                '神秘石段数': shenmishiduanshu, '翡翠石段数': feicuishiduanshu, '红宝石段数': hongbaoshiduanshu,'开运孔数': kaikong,
            ...
            print ("\n\n")
    print ('已爬取%s, %s, %d页' %(server_name, kindid_to_name[kindid], total_pages))    
    fp.close()
